...
server does not inspect the host's repository.

Contextual generation is available through `design_integration` (project-specific
Jev code) and `design_questions` (state/questions alone), with optional `context`,
`goal` and `sample_state` strings. The host inspects the relevant project and writes
the artifacts. Integration generation includes state construction, questions, SDK
calls, result composition, failure handling, application wiring and mocked tests;
production calls the SDK directly. Question generation returns a matching
`{state, questions}` request, provenance, assumptions and offline lint.
`decompose_judgment` retains its arguments and reuses question generation.

The shared `integration-design.md` and `question-design.md` guides are embedded in
the relevant prompts, also in `tenbin` for routing by the conversation, and synced
into the skill. Supplied arguments are kept in a separate JSON message. The MCP
does not implement an additional generative model backend. Generation and fake-client
tests need no key; live evaluation is separate and only runs when requested.
Offline mode exposes these four prompts, the linter and all resources. The two
remaining prompts and API tools still require a configured gateway.
...
flowchart LR
    ...
    subgraph MCP[tenbin]
        ...
        P[Prompts:<br/>tenbin<br/>design_integration / design_questions<br/>decompose_judgment<br/>design_thresholds]
    end
...
| `tenbin://guide/integration-design` | Contextual Jev code design/generation: project contracts, state/questions, SDK boundary, result handling, wiring and mocked tests |
| `tenbin://guide/question-design` | Contextual state/question design: field sources, complete request JSON, assumptions, synthetic samples, offline lint and trial cases |
...
|---|---|---|
| `tenbin` | none | Suggest uses grounded in project/conversation context; route explicit state/question or code-generation requests to the embedded design guides. Works offline |
| `design_integration` | `context?`, `goal?`, `sample_state?` | Design and generate project-native Jev integration code: linted state/questions, SDK calls, result logic, failure handling, application wiring and tests. Works offline |
| `design_questions` | `context?`, `goal?`, `sample_state?` | Suggest atomic judgments and state fields, then generate a complete request with sources, assumptions, type rationale and lint. Works offline |
| `decompose_judgment` | `judgment`, `sample_state?` | Compatible entry to question design; maps judgment to goal and returns the same workflow. Works offline |
...
All prompt arguments are strings; `sample_state` may contain JSON encoded as a
string or plain text. Send `arguments: {}` to use only current context in the two
new design prompts: the SDK requires an arguments object when a schema is registered.
The original `tenbin` prompt needs no arguments object. Draft-only requests stop at
lint; code-generation requests continue through local edits/checks; design-only
requests return the concrete design. Missing context is requested, not fabricated.
...
│   ├── integration-design.md     # Project-specific Jev integration code and verification (synced)
│   ├── question-design.md        # Contextual state/question generation and lint (synced)
...
For Jev code design/generation, follow `reference/integration-design.md` through
application wiring and mocked tests. For state/questions alone, follow
`reference/question-design.md` through the complete draft and lint. Live evaluation
is not a prerequisite for generation; unmeasured thresholds stay provisional.
